        ...
        for (uint32_t i = 0; i < entity_count; ++i) {
            ...
            for (uint32_t j = 0; j < comp_count; ++j) {
                ...
                if (type_idx == 0) {
                    ...
                    entity.add_component(Health(current, max_hp));
                } else if (type_idx == 3) {
                    int gold, items_count;
...
public:
    ...
    EntityBuilder& with_health(int current, int max_hp) {
        entity.add_component(Health(current, max_hp));
        return *this;
    }
...
int main() {
    ...
    for (EntityID i = 0; i < 100; ++i) {
        ...
        entity->add_component(Inventory(static_cast<int>(i) * 10, static_cast<int>(i) % 5));
    }
    ...
    *hero = built_entity;
    ...
    world.serialize_to_file("/tmp/game_state.bin");
    std::cout << "   Saved world state." << std::endl << std::endl;

    std::cout << "5. Game State Deserialization (Load):" << std::endl;
    GameWorld loaded_world;
    loaded_world.deserialize_from_file("/tmp/game_state.bin");
