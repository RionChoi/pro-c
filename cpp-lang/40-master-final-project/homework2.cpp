...
int main() {
    ...
    std::cout << "v1 · v2 = " << std::fixed << std::setprecision(2) << dot_product << std::endl;
    ...
    std::cout << "6. Advanced Vector Operations (2D):" << std::endl;
    Vector<2> p1{3.0, 4.0};
    Vector<2> p2{6.0, 8.0};
    ...
    Vector<2> displacement = p2 + (p1 * -1.0);
    displacement.print("p2 - p1");

    double distance = displacement.magnitude();
    std::cout << "Distance between p1 and p2: " << std::fixed << std::setprecision(2) << distance << std::endl;

    std::cout << "\n=== Numerical Engine Complete ===" << std::endl;
